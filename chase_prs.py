...
def venture_to_lines(file_name):
    ''' use pdf plumber for tables and get stuff from the v-card. '''
    # begin on line [:2] the 3rd page in after the summary top and the transactions.
    pass
...
def translate_to_csv(lines):
    ...
    for row in lines:
        ...
        print(f" each line broke down: desc = {desc}  date = {date}  amount = {amount}")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--output",
        default='Extracted_Chase_per_19_{}.csv'.format(dt.today().strftime('%Y%m%d')),  ### <<<<<<<<<<<<<<<< FILENAME
    ...
    per_checking = []  # compares with 8239
    biz_checking = []  # compares with 5281
    venture_card = []  # items from venture 7368
    
    input_files = []  # collect ALL PDFs
    for dirpath, subdirs, files in os.walk(args.dir+'\\statements\\Chase\\PersonalChk\\2019\\'):  # BusinessChk  PersonalChk
        for fl in files:
            if fl.endswith('.pdf'):
                fullfile = os.path.join(dirpath, fl)
                input_files.append(fullfile)
                if personal in fullfile:
                    per_checking.append(fullfile)
                if biz in fullfile:
                    biz_checking.append(fullfile)
                if venture in fullfile:
                    venture_card.append(fullfile)
                
                        
    
    # pause = input(f" waiting here for the dirs:")
